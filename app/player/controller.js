...
module.exports = {
  ...
  checkout: async (req, res) => {
    try {
      const { accountUser, name, nominal, voucher, payment, bank, buktiBayar } =
        req.body

      const res_voucher = await Voucher.findOne({ _id: voucher })
        .select('name category _id thumbnial user')
        .populate('category')
        .populate('user')
          ...
          const payload = {
            historyVoucherTopup: {
              ...
              category: res_voucher._doc.category
                ...
                : '',
              thumbnail: res_voucher._doc.thumbnial,
  ...
  topGamePayment: async (req, res) => {
    try {
      const paymentTop = await Transaction.aggregate([
        { $match: { player: req.player._id } },
        {
          $group: {
            _id: '$historyVoucherTopup.gameName',
            value: { $sum: '$value' },
            thumbnial: { $first: '$historyVoucherTopup.thumbnail' },
          },
        },
        { $sort: { value: -1 } },
        { $limit: 3 },
